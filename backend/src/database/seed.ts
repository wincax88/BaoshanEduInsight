...
async function seed() {
  ...
  const l1Repo = AppDataSource.getRepository(IndicatorL1);
  const l2Repo = AppDataSource.getRepository(IndicatorL2);
  const l3Repo = AppDataSource.getRepository(IndicatorL3);
  const itemRepo = AppDataSource.getRepository(EvaluationItem);
